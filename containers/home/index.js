...
function HomeContainer({ selectedCategory }) {
  const foundGenre = Genres.genres.find((genre) => genre.id === selectedCategory.id);
if (foundGenre) {
  console.log(foundGenre.name); // Doğru türün adını konsola yazdırın
} else {
  console.error("Genre not found!"); // Eğer uygun tür bulunamazsa hata mesajını yazdırın
}
